...
export class BomPanel extends LitElement {
  ...
  connectedCallback(): void {
    ...
    this.unsubscribe = cabinetStore.subscribe(state => {
      ...
      // Reload BOM when config changes and we have a layout
  ...
  private async loadBillOfMaterials(): Promise<void> {
    this.isLoading = true;
    this.error = null;

    try {
      const markdown = await api.getBillOfMaterials(this.cabinetState.config);
      this.markdown = markdown;
    } catch (err) {
      this.error = err instanceof Error ? err.message : 'Failed to load bill of materials';
    } finally {
    ...
    if (!this.cabinetState.layout) {
      return html`
        <div class="empty-state">
          ...
          <span>No bill of materials available</span>
          <small>Configure cabinet dimensions to generate a BOM</small>
        </div>
      `;
    }
      ...
      return html`
        <div class="error-state">
          ...
          <sl-button
            ...
            variant="primary"
            size="small"
            @click=${this.loadBillOfMaterials}
          >
            Retry
          </sl-button>
        </div>
